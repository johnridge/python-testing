...
class EpidemicModel:
    def __init__(self, I0, N0, Lambda, mu, beta, gamma, epsilon, omega, nu, delta):
        self.I0 = I0
        self.N0 = N0
        self.Lambda = Lambda
        self.mu = mu
...
def main(modelType, tMax, resolution, I0, N_0, Lambda, mu, beta, gamma, epsilon, omega, nu, animatePlot):
    ...
    def makeAnimation(model, compartments, labels, colors):
        ...
        for x in range(len(model.t)):
            ...
            for x in axs:
                x.grid(axis = 'both', alpha = 0.75)
                x.set(xlabel = 'Time')
                x.set(ylabel = 'Individuals')
            camera.snap()
    ...
    plt.show()
    if animatePlot:
        makeAnimation(modelType, model, compartments, labels, colors)
